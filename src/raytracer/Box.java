...
public class Box extends Geometry {
    ...
    public boolean intersect(Ray inRay, Ray outRay) {
        ...
        } else if (outRay.origin.x >= (maximum.x - Tracer.TOLERANCE)) {
            outRay.direction.set(1.0, 0.0, 0.0);
        } else if (outRay.origin.y <= (minimum.y + Tracer.TOLERANCE)) {
            outRay.direction.set(0.0, -1.0, 0.0);
        } else if (outRay.origin.y >= (maximum.y - Tracer.TOLERANCE)) {
            outRay.direction.set(0.0, 1.0, 0.0);
        } else if (outRay.origin.z <= (minimum.z + Tracer.TOLERANCE)) {
            outRay.direction.set(0.0, 0.0, -1.0);
        } else if (outRay.origin.z >= (maximum.z - Tracer.TOLERANCE)) {
            outRay.direction.set(0.0, 0.0, 1.0);
        }
